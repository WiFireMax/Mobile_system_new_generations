...
% Задаем координаты базовой станции (srsenb) и пользовательского оборудования (srsue)
srsenb_pos = [50, 50]; % Координаты базовой станции (x, y) в метрах
srsue_pos = [200, 150]; % Координаты пользовательского оборудования (x, y) в метрах

% Вычисляем расстояние между элементами сети
distance = sqrt((srsue_pos(1) - srsenb_pos(1))^2 + (srsue_pos(2) - srsenb_pos(2))^2);
fprintf('Расстояние между srsenb и srsue: %.2f метров\n', distance);

% Определение расстояний для расчета (можно использовать одно расстояние)
distances = [distance]; % Используем вычисленное расстояние
